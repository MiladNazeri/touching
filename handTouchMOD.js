...
(function(){
    ...
    Messages.subscribe("entity_channel");

    function handleMessages(channel, message, sender){
        print("channel", channel);
        print("message", message);
        var data = JSON.parse(message);
        print("!@# ID:" + data.entityID);
        if (channel === "entity_channel"){
    ...
    function updateSphereHand(side) {
        ...
        if (grabbables.length > 0) {
            ...
            if (intersection.intersects === true && intersection.entityID === entityID){
                Messages.sendMessage(MESSAGE_CHANNEL, JSON.stringify(intersection));
            }
